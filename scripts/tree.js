...
(function (g, $) {
	...
	*/
	...
	abTree.prototype.addHoverDom = function (treeId, treeNode) {
		...
		if (btnAdd) btnAdd.on('click', function() {
			var name,
				path,
				ok = false,
				i = 1;
		...
		if (btnRemove) btnRemove.on('click', function() {
		...
		if (btnRename) btnRemove.on('click', function() {
	...
	abTree.init = function(params) {
		...
		self.promise = s3.getObject(params).promise()
			...
			.then( function(zNodes) {
				...
				abUtils.attachTouchToMoveListeners($abTree, 0, 1500);
